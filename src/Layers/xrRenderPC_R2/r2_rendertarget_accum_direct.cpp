...
//		float			fBias				= (SE_SUN_NEAR==sub_phase)?(-ps_r2_sun_depth_near_bias):ps_r2_sun_depth_far_bias;
		...
		// See the cross-fade in accum_sun_cascade.ps / accum_sun_cascade_far.ps.
		...
		// and a closed box can then rasterise TWICE -- entry and exit face -- for everything in
		// front of it. That was harmless while the passes overwrote each other; with additive
		// accumulation it is a straight double dose of sunlight. A screen-filling quad gives one
		// fragment per pixel by construction, and costs nothing here because the far pass runs
		// with ZFUNC ALWAYS anyway. (With SUN_ZCULLING on the depth test is real, so that path
		// keeps the box.)
		...
		if ( b_screen_quad )
		{
			...
			FVF::L* pv				= (FVF::L*)	RCache.Vertex.Lock	( 4,g_combine_cuboid.stride(),Offset);
			for ( u32 i = 0; i < 4; ++i )
			{
				Fvector3 tmp_vec;
...
#			define FOURCC_GET4  MAKEFOURCC('G','E','T','4') 
		...
		if ( b_screen_quad )
		{
			// a single quad is not a closed volume, so culling must not eat it
			RCache.set_CullMode		(CULL_NONE);
