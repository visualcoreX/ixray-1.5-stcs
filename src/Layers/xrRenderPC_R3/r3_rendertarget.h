...
public:
   ...
   ref_rt                  rt_Generic;
	...
	// 3D PDA: PDA window snapshot, sampled by the model's screen material as $user$ui. Always
	// 1-sample: it gets sampled in a shader, so it can't be an MSAA target.
	ref_rt						rt_ui;
...
public:
	...
	void						accum_direct_cascade	(u32	sub_phase, Fmatrix& xform, Fmatrix& xform_prev, Fmatrix& xform_prev2, float fBias, const Fvector4& blend );
