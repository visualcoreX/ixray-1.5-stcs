...
#include "../missile.h"
#include "../Actor_Flags.h"			// AF_SHOW_QUICK_SLOTS: hud quick-slot icons on/off
#include "../Grenade.h"
...
void CUIMainIngameWnd::UpdateQuickSlots()
{
	...
	// Options checkbox / `hud_show_quick_slots`: with it clear the whole display comes off the hud.
	// Done here rather than by skipping the draw so the key labels and counters go with it, and so
	// turning it back on needs nothing but the next frame. The slots themselves are untouched --
	// the keys still work.
	if (!psActorFlags.test(AF_SHOW_QUICK_SLOTS))
	{
