...
public:
	...
	void 					Render				();
	// tracers render in the R3 forward phase (correct depth vs HUD) when MSAA is on; this flag lets
	// the later UI-pass render skip them so they aren't drawn twice.
	bool					m_bRenderedThisFrame = false;
	bool					WasRenderedThisFrame() const { return m_bRenderedThisFrame; }
};
