...
void CInventory::SetSlotsBlocked(u16 mask, bool bBlock)
{
	...
	for(u32 i =0; i<m_slots.size(); ++i)
	{
		// the artefact slot (10) hosts the animated item-use phantom (gwr_eatable); never block it,
		// so eat/use anims still draw in safe zones where weapons are hidden. Slot 10 is otherwise
		// unused in single-player, so this has no other effect.
		if(i == ARTEFACT_SLOT)
			continue;
		if(mask & (1<<i))
		{
